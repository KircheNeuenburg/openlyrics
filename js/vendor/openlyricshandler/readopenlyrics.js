...
function  readOpenLyrics () {}

'use strict';

  // DOM Parser
  readOpenLyrics.parse_dom = function(xml) {
  ...
  readOpenLyrics.get_metadata = function (xml, name) {
    var names = undefined;
  ...
  // Titles
  readOpenLyrics.get_titles = function(xml) {
    var names = [];
  ...
  readOpenLyrics.titlesToString = function(a, lang) {
    var titles = [];
    if (lang) {
  ...
  // Authors
  readOpenLyrics.get_authors = function (xml) {
    var names = [];
  ...
  readOpenLyrics.authorsToString = function(a) {
    var names = [];
  ...
  // Songbooks
  readOpenLyrics.get_songbooks = function(xml) {
    var names = [];
  ...
  // Themes
  readOpenLyrics.get_themes = function(xml) {
    var names = [];
  ...
  // Comments
  readOpenLyrics.get_comments = function(xml) {
    var names = [];
  ...
  // format Chord  
  readOpenLyrics.formatChordDiv = function(chord) {
    chord.substring(1) ? chord = chord.charAt(0) + '<small>' + chord.substring(1).replace(/([0-9]+)/g, '<sup>$1</sup>') + '</small>' : chord = chord.charAt(0);
    return chord;
  };
  
  readOpenLyrics.formatChord = function(chord, transp) {
    if (transp) { // Transposer
      if (chord.indexOf("/") >= 0) { // Handle Chords with "/" e.g. "E/D"
        chord = chord.split("/");
        return  readOpenLyrics.formatChordDiv(Transposer.transpose(chord[0], transp)) + "/" +  readOpenLyrics.formatChordDiv(Transposer.transpose(chord[1], transp));
      } else {
        return  readOpenLyrics.formatChordDiv(Transposer.transpose(chord, transp));
      }
    } else {
      if (chord.indexOf("/") >= 0) { // Handle Chords with "/" e.g. "E/D"
        chord = chord.split("/");
        return  readOpenLyrics.formatChordDiv(chord[0]) + "/" +  readOpenLyrics.formatChordDiv(chord[1]);
      } else {
        return  readOpenLyrics.formatChordDiv(chord);
      }
    }
  };
  ...
  readOpenLyrics.parselines = function(line, haschords, showChords, showComments, transp) {
      ...
      if (haschords && showChords) { 
        if (lineArray[j].nodeValue === null) { // node is htmltag
          ...
          if (lineArray[j].tagName === "chord") {
            ...
            lines = lines + "<div class='" + moPrefix + "chordbox'><div class='" + moPrefix + "chord'>"+  readOpenLyrics.formatChord(chord, transp) + "&nbsp;" + "</div>";
  ...
  readOpenLyrics.get_lyrics = function(xml, vo, showChords, showComments, transp) {
    var data = {}
    ...
    if (l[0] != undefined) {
      ...
      for (i = 0, len9 = l.length; i < len9; i++) { // [verses]
        ...
        for (m=0, len11 = line.length; m < len11; m++) {
          ...
          lines +=  readOpenLyrics.parselines(line[m].childNodes, data.haschords, 
  ...
  // Parse OpenLyric 0.8
  readOpenLyrics.parse = function(xml, showChords, showComments, transp) {
    var data = {};
...
// Parse for editing
  readOpenLyrics.allMetadata = function(xml) {
    var data = {};
  ...
  readOpenLyrics.editLyrics = function(xml) {
    var data = {};
    var l = xml.getElementsByTagName("verse");
